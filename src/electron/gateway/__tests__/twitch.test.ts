...
describe('TwitchAdapter', () => {
  ...
  describe('editMessage', () => {
    it('should throw error as Twitch does not support editing', async () => {
      await expect(adapter.editMessage('testchannel', 'msg-123', 'Updated'))
        .rejects.toThrow('Twitch does not support message editing');
    });
  });

  describe('deleteMessage validation', () => {
    it('should throw error as deletion is not implemented', async () => {
      await expect(adapter.deleteMessage('testchannel', 'msg-123'))
        .rejects.toThrow('Twitch message deletion not implemented');
    });
  });

  describe('sendDocument validation', () => {
    it('should throw error as Twitch does not support files', async () => {
      await expect(adapter.sendDocument('testchannel', '/path/to/file.pdf'))
        .rejects.toThrow('Twitch does not support file attachments');
    });
  });

  describe('sendPhoto validation', () => {
    it('should throw error as Twitch does not support images', async () => {
      await expect(adapter.sendPhoto('testchannel', '/path/to/image.png'))
        .rejects.toThrow('Twitch does not support image attachments');
    });
  });
  ...
  describe('sendWhisper validation', () => {
    it('should throw error when not connected', async () => {
      await expect(adapter.sendWhisper('targetuser', 'Hello'))
        .rejects.toThrow('Twitch client is not connected');
    });
  });
});
