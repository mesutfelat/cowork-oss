...
describe('MatrixAdapter', () => {
  ...
  describe('editMessage', () => {
    it('should warn that editing is not fully supported', async () => {
      await expect(adapter.editMessage('!room:matrix.org', '$event123', 'Updated'))
        .rejects.toThrow('Matrix message editing not fully supported');
    });
  });

  describe('deleteMessage validation', () => {
    it('should throw error when not connected', async () => {
      await expect(adapter.deleteMessage('!room:matrix.org', '$event123'))
        .rejects.toThrow('Matrix client is not connected');
    });
  });
  ...
  describe('sendDocument validation', () => {
    it('should throw error when not connected', async () => {
      await expect(adapter.sendDocument('!room:matrix.org', '/path/to/file.pdf'))
        .rejects.toThrow('Matrix client is not connected');
    });
  });
  ...
  describe('sendPhoto validation', () => {
    it('should throw error when not connected', async () => {
      await expect(adapter.sendPhoto('!room:matrix.org', '/path/to/image.png'))
        .rejects.toThrow('Matrix client is not connected');
    });
  });
});
