...
import type { AgentRole, HeartbeatEvent, AgentMention, Task, Activity } from '../../../shared/types';
...
describe('HeartbeatService', () => {
  ...
  beforeEach(() => {
    ...
    deps = {
      ...
      createTask: async (workspaceId: string, prompt: string, title: string, agentRoleId?: string) => {
        const task: Task = {
  ...
  describe('concurrent execution', () => {
    it('should prevent concurrent heartbeat execution', async () => {
      ...
      deps.createTask = vi.fn().mockImplementation(() => new Promise<Task>((resolve) => {
        resolveTask = resolve;
      }));
  ...
  describe('scheduled execution', () => {
    it('should execute heartbeat after interval', async () => {
      ...
      expect(heartbeatEvents.some(e => e.agentRoleId === 'agent-1' && e.type === 'completed')).toBe(true);
    });
  });
});
