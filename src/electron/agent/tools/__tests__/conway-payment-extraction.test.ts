...
import { extractPaymentAmountFromX402Tool, getConwayPaymentLimitError } from '../registry';
...
describe('extractPaymentAmountFromX402Tool', () => {
  ...
  it('extracts amount from nested request.amount when schema defines it', () => {
    expect(extractPaymentAmountFromX402Tool({ request: { amount: '75.25' } }, nestedRequestTool))
      .toBe(75.25);
  });
  ...
  it('returns default cap error when amount is over safety limit', () => {
    ...
    expect(getConwayPaymentLimitError({ amount: 150 }, amountTool))
      .toMatch(/Conway payment amount is above safety cap/);
  });
  ...
  it('returns configured cap error when amount exceeds env override', () => {
    ...
    const previous = process.env.COWORK_CONWAY_PAYMENT_LIMIT_USD;
    process.env.COWORK_CONWAY_PAYMENT_LIMIT_USD = '50';

    try {
      expect(getConwayPaymentLimitError({ amount: 75 }, amountTool))
        .toMatch(/exceeds configured cap of 50/);
    } finally {
      if (previous === undefined) {
        delete process.env.COWORK_CONWAY_PAYMENT_LIMIT_USD;
      } else {
        process.env.COWORK_CONWAY_PAYMENT_LIMIT_USD = previous;
      }
    }
  });
  ...
  it('allows x402 amount within limits', () => {
    ...
    const previous = process.env.COWORK_CONWAY_PAYMENT_LIMIT_USD;
    process.env.COWORK_CONWAY_PAYMENT_LIMIT_USD = '75';

    try {
      expect(getConwayPaymentLimitError({ maxAmount: 50 }, amountTool)).toBeNull();
    } finally {
      if (previous === undefined) {
        delete process.env.COWORK_CONWAY_PAYMENT_LIMIT_USD;
      } else {
