...
describe("extractPaymentAmountFromX402Tool", () => {
  ...
  it("returns default cap error when amount is over safety limit", () => {
    ...
    expect(getConwayPaymentLimitError({ amount: 150 }, amountTool)).toMatch(
      /Conway payment amount is above safety cap/,
    );
  });
  ...
  it("returns configured cap error when amount exceeds env override", () => {
    ...
    try {
      expect(getConwayPaymentLimitError({ amount: 75 }, amountTool)).toMatch(
