...
describe('Conversation Snapshot', () => {
  describe('restoreFromSnapshot', () => {
    ...
    it('should return the most recent snapshot when multiple exist', () => {
      const oldHistory: ConversationMessage[] = [
  ...
  describe('buildLegacySummary', () => {
    ...
    it('should truncate long assistant messages', () => {
      ...
      const events: TaskEvent[] = [
        createMockEvent('assistant_message', { message: longMessage }),
      ];
    ...
    it('should handle error events', () => {
      const events: TaskEvent[] = [
        createMockEvent('error', { message: 'Something went wrong' }),
      ];
  ...
  describe('Conversation persistence scenarios', () => {
      ...
      const fullHistory: ConversationMessage[] = [
        { role: 'user', content: 'What is the latest news about AI?' },
        {
          ...
          content: [
            ...
            { type: 'tool_use', id: 'tool-1', name: 'web_search', input: { query: 'latest AI news' } },
          ],
        },
        {
          role: 'user',
        ...
        {
          role: 'assistant',
          content: [{ type: 'text', text: 'Here are the latest AI news: 1. New breakthrough in language models...' }],
        },
      ];

      const events: TaskEvent[] = [
        createSnapshotEvent(fullHistory, 'System prompt'),
      ];
    ...
    it('should handle conversation after app restart', () => {
      ...
      const events: TaskEvent[] = [
        ...
        createMockEvent('assistant_message', { message: 'I found 3 main modules...' }, Date.now() - 30000),
        createSnapshotEvent(originalHistory, 'System prompt', Date.now() - 10000),
      ];

      // After restart, rebuild should restore full conversation
      const result = rebuildConversationFromEvents(events, 'Analyze codebase', 'Analyze the codebase structure');

      expect(result.restoredFromSnapshot).toBe(true);
      expect(result.conversationHistory).toHaveLength(2);
      expect(result.conversationHistory[1].content).toEqual([{ type: 'text', text: 'I found 3 main modules...' }]);
    });
...
describe('Snapshot pruning logic', () => {
  ...
  function findSnapshotsToDelete(
...
describe('FileOperationTracker serialization', () => {
  ...
  it('should serialize tracker state correctly', () => {
    const state = {
      ...
      createdFiles: new Map([
        ['normalized-name', '/path/to/created.ts'],
      ]),
      directories: new Map([
  ...
  it('should limit serialized data to prevent huge snapshots', () => {
    const state = {
      readFiles: new Map(
        Array.from({ length: 100 }, (_, i) => [`/file${i}.ts`, { count: 1, lastReadTime: Date.now(), contentLength: 100 }])
      ),
      createdFiles: new Map(
        Array.from({ length: 100 }, (_, i) => [`name${i}`, `/created${i}.ts`])
      ),
      directories: new Map(
        Array.from({ length: 50 }, (_, i) => [`/dir${i}`, { files: [], lastListTime: Date.now(), count: 1 }])
      ),
    };
