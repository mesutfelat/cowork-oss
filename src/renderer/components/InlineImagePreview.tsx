...
export function InlineImagePreview({ filePath, workspacePath, onOpenViewer }: InlineImagePreviewProps) {
  ...
  return (
    <div className="inline-image-preview">
      {loading && (
        <div className="inline-image-preview-loading">Loading image…</div>
      )}
      ...
      {!loading && !error && result?.content && (
        <>
          ...
          {meta && (
            <div className="inline-image-preview-meta">{meta}</div>
          )}
        </>
      )}
    </div>
